...
void Calc::result() {
  double res = 0.0;
  s21::Parser parser;
  s21::Calculator calculator(parser);

  if (ui->exprLine->text().contains("x")) {
    try {
      res = calculator.calculate(ui->exprLine->text().toLocal8Bit().data(),
    ...
    try {
      res = calculator.calculate(ui->exprLine->text().toLocal8Bit().data(), 0);
      ui->exprLine->setText(QString::number(res, 'g', 8));
    } catch (const std::exception &e) {
      std::cerr << e.what() << '\n';
      ui->exprLine->setText("Error");
...
void Calc::graph() {
  ...
  graph->scailing();
  try {
    graph->plot(ui->exprLine->text().toLocal8Bit().data());
